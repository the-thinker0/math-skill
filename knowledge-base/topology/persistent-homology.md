...
- **可微替代**：persistence map 对点位置几乎处处可微（不可微点是滤过值退化/重排的正则值例外集，测度为零）；persistence image/landscape 给出光滑向量化，barcode 本身的离散匹配结构（bottleneck 匹配）不光滑
...
- **可微性的正确表述**：persistence map 几乎处处可微，可微拓扑层（可微 TopoLoss 类）成立；不可微发生在滤过值退化/重排处与 bottleneck 匹配的离散结构上。需要端到端梯度时用 landscape/image 等光滑代理
