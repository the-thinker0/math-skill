...
一句话激活观点：**对称性是需要任务验证的归纳偏置**。把群 G 的作用写入结构，可通过参数共享限制函数空间；只有任务的输入、标签和输出确实遵循该对称性时，这一限制才可能有益。参数减少量由表示和共享轨道决定，泛化收益还依赖数据与学习假设，不由群公理自动保证。群作用、半环和有限域分别为等变结构、动态规划与编码提供不同工具，不能仅凭代数名称推断硬件收益。
...
| **商/因子结构 quotient**（9, 14）| 群商 G/N 要求 N 正规，环商要求理想；群作用的轨道空间 X/G 是另一概念 | 置换不变聚合在输入轨道空间 X/Sₙ 上定义；不能称为“对 Sₙ 取商群” |
| **循环 & 有限阿贝尔群**（4, 11）| Zₙ 的周期结构，任意有限阿贝尔群 ≅ 循环群直积 | DFT/FFT、循环卷积；RoPE 可看作 Z 到 SO(2) 旋转块的表示，不必有有限共同周期|
| **环 ring**（12–14）→ 放松公理 | 把加法放松为交换幺半群并显式保留零吸收等公理 → **半环 semiring**（教材之外的扩展）| 广义矩阵乘：把 (×,+) 换成 (+, min/max) 的 **min-plus / 热带半环 tropical semiring** GEMM |
| **有限域 GF(pⁿ)**（22, 16, 17）| 元素有限、算术封闭精确的域 | 纠错码、通用哈希、CRC 的代数、Shamir 秘密共享 |
| **线性码 linear code**（31）| 有限域 F 上的 k 维子空间，生成矩阵 G、Hamming 距离/权重 | 鲁棒存储/通信、梯度压缩、容错训练、安全聚合 |

### 三个最值得激活的映射（展开）

- **群作用 → 等变 + 权重共享**：有限群的轨道-稳定子定理为 |G·x|=|G|/|Gₓ|。对置换表示的线性层，共享通常发生在输入—输出索引对的轨道上；独立参数数由这些轨道数决定，不普遍减少 |G| 倍。一般表示须满足 Wρ_in(g)=ρ_out(g)W。CNN/G-CNN、集合模型和 E(3)-等变网络还各自需要兼容的非线性与输出表示。
- **环公理松弛 → 半环 → 广义 GEMM**：本书 12–14 章给出环的两条运算 (+,×) 与公理；半环把加法群放松为交换幺半群，同时须明确保留结合律、分配律和零吸收律；采用含幺约定时还保留乘法单位元。把通用矩阵乘 C=A⊗B 里的 (×,+) 抽象成任意半环 (⊙,⊕)，一套代码即可同时表达标准 GEMM、布尔可达性 (∧,∨)、最短路/Viterbi/DTW (+, min)。这是把"动态规划"装进矩阵乘的统一视角。
- **有限域 → 精确可重放算术**：GF(pⁿ)（22 章，由多项式环模不可约多项式构造，见 16–17 章）的算术封闭且**无浮点误差**，天然适合需要可重放、可验证、可纠错的场景：Reed-Solomon / Hamming 纠错码（31 章）、Shamir 秘密共享（GF(p) 多项式插值）、部分通用哈希族。有限域算术本身不保距离局部性，也不自动给出 LSH 构造；整数溢出和模约减仍须正确实现。
...
1. **群等变层 G-equivariant layer**（源：5, 6, 10）：先定义输入与输出表示，再解 Wρ_in(g)=ρ_out(g)W 的线性约束。例如 n≥2 时，实数 n×n 矩阵对所有坐标置换等变当且仅当 W=aI+b11ᵀ，只需两个标量；这已说明参数数不是简单除以 n!。非线性层还需验证复合后等变性。对变换输入直接平均通常得到不变表示，不能不加输出变换校正就宣称等变。
2. **热带 / min-plus 注意力与路由**（源：12–14 的环公理启发，扩展到半环）：score_ij=min_k(Q_ik+K_kj) 改变了点积相似度的语义。min/max 值几乎处处可微，tie 处可约定广义梯度；离散 argmin/argmax 索引则不因值可微而可微。log-sum-exp 是可选平滑近似，其梯度是 softmax 权重，但既不等于原来的硬选择，也不会把 min-plus 矩阵乘变成普通 GEMM。需对照 `../gpu-friendly-math.md` 测专用 kernel、温度偏差与梯度质量。
...
6. **群上的傅里叶 / 谱方法**（从 10 同态 + 4/11 继续深入）：有限群表示论可给出群卷积的频域分块形式；SO(3) 的调和分析还涉及紧群表示论，超出本教材的基础范围。一般图 Laplacian 的谱分解并不自动是群傅里叶变换。快速算法与变换成本须按具体群另证。
...
- **数值与梯度**：max/min 避免指数上溢，但加法、无穷元表示、tie 和低精度比较仍会出问题。max/min 几乎处处可微，tie 处可选择 Clarke 广义梯度；log-sum-exp 是平滑近似，会改变精确半环语义，并不“退回普通 GEMM”等价计算。
...
- （半环松弛起点：Ch 12–14 环公理 → 放松加法并明确保留半环公理 → 热带例子。）

## 已核验的扩展来源

以上 AI 构造超出教材本身：[Ravanbakhsh 等：参数共享与等变性](https://proceedings.mlr.press/v70/ravanbakhsh17a.html)、[Cohen 与 Welling：G-CNN](https://proceedings.mlr.press/v48/cohenc16.html)。这些论文支持具体的等变构造，不支持无条件泛化或统一参数缩减倍数。
