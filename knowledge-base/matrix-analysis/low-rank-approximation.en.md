...
- **KV-Cache low-rank reduction**: Maintain $K$ in factored low-rank form $K \approx U_k \Sigma_k V_k^H$ (storing $U_k \in \mathbb{R}^{L \times k}$ and $\Sigma_k V_k^H \in \mathbb{R}^{k \times d}$, totaling $O(Lk + kd)$ instead of $O(Ld)$). Perform incremental PCA or streaming SVD updates for each new token. **Note**: for standard softmax attention, one must first reconstruct $K_k = U_k (\Sigma_k V_k^H) \in \mathbb{R}^{L \times d}$, so the sequence dimension remains $L$ (saves memory but not compute). Only with linear attention (kernel feature map $\phi$) can the factored form be exploited to compute $\phi(Q)(\phi(K_k)^T V_k)$, reducing the sequence dimension from $L$ to $k$ and saving both memory and compute.
