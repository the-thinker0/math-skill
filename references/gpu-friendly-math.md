...
评级标记：`[v]` 友好（可张量化/开销可忽略）、`[~]` 部分可行（需近似/近似质量决定效果）、`[x]` 不友好（串行/不可物化/显存不可行）。
...
1. **数学正确（beautiful in math）**——结构自洽，结论有相应条件/证据；只有梯度路径要求可微或合适的梯度估计，推理与离散算法不必可微。
...
- **梯度路径中的离散步骤**：需明确梯度估计/松弛或把它移出梯度路径；不可微不等于不可计算。

## 改造手法 / Make-It-Computable Toolkit

把"美但不可算"改造成"既美又可算"的常用招式：

- **离散 → 连续松弛**：Gumbel-softmax 或任务适配的软排序松弛。max-plus 门控是另一类分段线性设计，并不自动近似 Top-K 或保留严格稀疏性。
- **块稀疏化**：块内 dense attention、块间结构化 sparse。
- **低秩 / 投影压缩**：限制映射（restriction map）用低秩线性变换；以**基底加系数**压缩 KV-Cache，或明确采用有损块摘要。两组因子都要计入存储；只有子空间不能重建任意 K/V，Plücker 坐标的组合数开销取决于环境维数和秩。
- **数值重参数化**：log-sum-exp、归一化与稳定 softmax，减少已知数值风险；仍需精度/动态范围测试。
- **算子融合 / 重计算**：融合 kernel、activation recompute 省显存。
- **结构嵌入 GEMM**：把代数/几何变换写成**可学习线性映射**，再按数据类型、尺寸和 kernel 确认是否实际使用 Tensor Core。

## 范例对照：Tropical Sheaf Attention / Worked Example

以下探索性组合演示一个**候选设计如何进入八维验证**，不是已发表方法或实验结果：
...
| 胞腔层扩散 Cellular Sheaf Diffusion | 代数几何/拓扑（层、限制映射）| 边限制映射可采用低秩线性变换；除 GEMM 外，还需计入系数、gather、scatter-add 与小 kernel 开销（D2/D4/D7）|
| 胞腔一致性正则 | 代数拓扑（上边缘算子 $d^0$）| $\|d^0x\|^2$ 只衡量给定限制映射下的兼容性，不代表事实正确性或幻觉检测；精确 $H^1$ 还需适当的复形与单独计算 |
| 低秩基底 KV 压缩（Plücker/Grassmannian 视角） | 射影几何 | 重建 K/V 时需存基底加系数；只有子空间摘要会丢失信息。计入全部存储，再实测输出误差和吞吐（D4）|
