...
**使用前检查**：Markov 要求 $a>0$ 且期望有限；Hoeffding 要求独立 $X_i\in[a_i,b_i]$ 几乎处处；McDiarmid 要求输入独立及一致有界差分条件。指数集中依赖分布/相关性假设，不因随机项很多就自动成立。矩阵集中是单独的定理，不能无维数因子地把标量界逐元素套用。
...
## AI 设计翻译
- **泛化**：固定预测器的集中界不自动覆盖用同一批样本拟合的预测器；需一致界/复杂度度量、稳定性论证或独立留出协议。
- **随机正则**：约束 dropout/stochastic-depth 输出前，须明确独立性、有界增量或尾部假设及非线性噪声传播。
- **梯度压缩**：集中界可约束明确定义的估计误差；分布式收敛另需优化假设、压缩偏差处理及通信/更新规则。
...
- **重尾**：无界重尾不能直接用有界变量 Hoeffding 或本卡有界变量 Bernstein。采用相应有限矩界、鲁棒估计器，或含明确偏差项的截断。
