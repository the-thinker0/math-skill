...
本项目的 16 种思想武器（15种专业武器 + 1个武器选择器）不是孤立存在的，它们与数学分支形成了互补的工具集。下面展示一个简化映射，帮助你从问题类型直接找到适合的数学方法。

| 思维武器 | 对应数学分支 | 典型方法/概念 |
|---------|-------------|--------------|
| 🧭 武器选择器 | 元认知、问题分类 | 决策树、问题特征匹配、工具推荐 |
...
如果不确定该用哪个工具，直接使用 `/ask` 命令调用武器选择器，它会根据问题的核心特征自动推荐1-3个最合适的思想武器。

### 模式选择

每个思想武器都有科研模式和生活模式：
- **科研模式**：保留完整数学符号，适用于数学推导、论文审查、实验设计
- **生活模式**：只保留思维框架核心步骤，用日常语言替代数学符号，适用于日常决策、人际互动、生活规划
...
- 根据问题性质选择科研模式或生活模式——科研问题用科研模式，生活问题用生活模式。
