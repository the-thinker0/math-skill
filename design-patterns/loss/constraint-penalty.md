...
方法2 - 增广拉格朗日（ALM）：
  ...
  // 比纯罚函数收敛快，避免 ρ → ∞
  对不等式约束 h(x) ≤ 0：
  L_ALM = Σ_j [λ_j · max(0, h_j(x) + λ_j/(2ρ)) + ρ/2 · max(0, h_j(x) + λ_j/(2ρ))² - λ_j²/(2ρ)]
  // 严格可行（h_j < -λ_j/(2ρ)）的约束不被惩罚
...
通过对偶变量 λ 的交替上升更新，在无需 ρ→∞ 的条件下保证约束满足度以 O(1/ρ) 收敛（对凸问题成立；非凸问题仅保证局部收敛到 KKT 点，约束满足度依赖于约束规格如 LICQ/MFCQ），
