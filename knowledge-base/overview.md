...
- 快速定位数学问题所属的学科与方法领域；
- 理解本项目 15 个思想透镜（15 武器 + math-research-activator）与数学分支之间的对应关系；
...
研究随机现象和数据分析，广泛应用于金融、生物、社会科学等。对应思想透镜：**概率与统计**、**信息论思想**、**因果推断思想**。
...
集合论、图论、数理逻辑等，是计算机科学的数学基础。对应思想透镜：**算法与计算思想**、**离散与组合思想**。
...
将数学方法应用于实际问题。对应思想透镜：**优化思想**、**信息论思想**、**博弈论思想**、**算法与计算思想**、**因果推断思想**。
...
本项目的 15 个思想透镜（15 种专业武器 + 1 个 math-research-activator 自动触发入口）不是孤立存在的，它们与数学分支形成了互补的工具集。下面展示一个简化映射，帮助你从问题类型直接找到适合的数学方法。v2 另有 `references/books/*.md` × 7 把代数几何 / 微分几何 / 李理论 / 抽象代数 / 矩阵分析 / 最优化 / 流形等现代数学结构按需激活进算法设计（见 README「三层渐进式披露」）。
...
如果不确定该用哪个工具，直接使用 `/ask` 命令调用 math-research-activator，它会诊断问题、映射现代数学结构、推荐 1–3 个最合适的思想透镜并做 GPU 可行性筛选。工作区出现算法 / kernel / 模型代码时，激活器也会自动介入。
...
- 对照快速选择指南或使用 `/ask` 命令，找到最贴近的思想透镜。
- 结合”知识体系层次结构”，判断当前问题更适合基础思路还是前沿方法。
- 若问题横跨多个类别，可同时调用多个思想透镜，例如"建模+优化"、"概率+因果"、"抽象+算法"。
