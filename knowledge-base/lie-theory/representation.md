...
群 $G$ 在向量空间 $V$ 上的表示是群同态 $\rho: G \to GL(V)$，将抽象群元素实现为可计算的线性变换（矩阵）。不可约直和分解要求完全可约性，例如有限群或紧群的有限维复表示；一般表示可能可约而不能分裂。
...
- **Schur 约束的权重矩阵**：复有限维完全可约表示下，等变映射在 isotypic 分块中形如 $A_\lambda\otimes I_{d_\lambda}$；重数空间可任意线性混合，不是整个块只能是一个标量。实表示还需区分其交换子代数。
- **特征标池化**：$\chi_\rho(hgh^{-1})=\chi_\rho(g)$ 保证对共轭作用不变；不能把特征标直接当任意群输入作用的不变量。

## 工程可行性

GPU 友好度取决于群的规模和表示维数：
- **有限群表示**：每个 irrep 是小矩阵（$d \times d$），群 Fourier 变换 = 一批小 GEMM，$O(|G| \cdot d^2)$，可 batched
- **球面与群变换**：$S^2$ 的球谐变换与 SO(3) 的 Wigner-D 变换不同；复杂度依带宽、采样规则与实现，不能统称 $O(L^2\log L)$。
- **Clebsch-Gordan 系数**：预计算后是固定的稀疏张量，与特征的缩并可 GEMM 化
- **Schur 分块**：在适当表示基中利用重数矩阵与 irrep 恒等因子，通常可用小型稠密 GEMM；不要求任意坐标基下显式稀疏。
...
- **非紧群**：可以有有限维但非酉表示；某些谐波分析需要无限维酉表示。Lorentz 群的所有不可约表示并非一律无限维。
