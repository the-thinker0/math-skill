...
- For an embedded manifold with the induced Euclidean metric, $\operatorname{grad}f(x)=\operatorname{proj}_{T_x\mathcal M}(\nabla f(x))$. For a different metric, solve $g_x(\operatorname{grad}f,\xi)=Df(x)[\xi]$; Euclidean projection alone is insufficient.
...
- Cayley retraction on $O(n)$: for $\xi=Q\Omega$, $\Omega^T=-\Omega$, use $R_Q(\xi)=Q(I-\Omega/2)^{-1}(I+\Omega/2)$. Then $R_Q(0)=Q$ and $\frac{d}{dt}R_Q(t\xi)|_{t=0}=\xi$. A descent step inserts $\xi=-\alpha\,\operatorname{grad}f(Q)$; reversing the Cayley signs changes the direction.
- Polar decomposition retraction: $R_Q(\xi) = (Q + \xi)(I + \xi^T\xi)^{-1/2}$ (projection onto the nearest orthogonal matrix)
- Newton–Schulz orthogonalization: $X_{k+1}=X_k(3I-X_k^TX_k)/2$ converges to the column polar factor for full-column-rank input with singular values initially in $(0,\sqrt3)$; scale first and check $\|X_k^TX_k-I\|$. Rank-deficient inputs cannot produce orthonormal columns by this iteration.
...
- **Low precision**: Newton–Schulz still depends on scaling, singular-value range and stopping tolerance; pure matmul does not make it bf16-stable. Check orthogonality and descent residuals. Near a Poincaré boundary the metric blows up while its inverse factor vanishes; resulting gradient behavior depends on the objective.
...
- **Normalization composition**: Orthogonality of a weight matrix does not make the whole network an isometry. BatchNorm, GroupNorm, learned scaling and nonlinearities can all alter the metric; switching normalization alone does not preserve orthogonality of the composed mapping.
