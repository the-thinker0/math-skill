...
- **Davis–Kahan（安全的前 $k$ 维版本）**：Hermitian $A,\hat A=A+E$ 的特征值降序，$\gamma=\lambda_k(A)-\lambda_{k+1}(A)>0$，且 $\|E\|_2<\gamma/2$ 时，前 $k$ 维子空间满足 $\|\sin\Theta(\hat U,U)\|_2\le2\|E\|_2/\gamma$。常数为1的版本需精确定义跨扰动前后谱的分离量，不能只写模糊的 gap。
- **奇异值扰动 (Mirsky)**：$|\sigma_i(A+E) - \sigma_i(A)| \leq \|E\|_2$
- **Ger\v{s}gorin 圆盘**：$\lambda_i(A) \in \bigcup_j \{z : |z - a_{jj}| \leq \sum_{k \neq j} |a_{jk}|\}$
- **线性系统扰动**：若 $Ax=b$、$(A+E)\hat x=b$、$A$ 可逆，且 $\kappa(A)\|E\|/\|A\|<1$，则 $\|\hat x-x\|/\|x\|\le\kappa(A)(\|E\|/\|A\|)/(1-\kappa(A)\|E\|/\|A\|)$。省略分母的乘积只是一阶近似。
...
- **量化扰动**：奇异值漂移至多 $\|E\|_2$。对无偏置线性层与满足 $\phi(0)=0$ 的1-Lipschitz 激活复合，一个保守输出界是 $\|x\|[\prod_i(\|W_i\|_2+\|E_i\|_2)-\prod_i\|W_i\|_2]$。偏置、归一化、注意力及残差分支需单独传播项。
...
- **谱鲁棒代理**：惩罚 $\max(0,\|E\|_2-\epsilon)^2$ 抑制给定扰动，不证明对抗集合上的最坏情况。有限幂迭代通常给谱范数的下估计；证书需有效上界及优化/舍入误差核算。
...
- **低精度**：扰动定理针对精确矩阵与范数；计算残差/范数估计具有依赖尺度及算法的误差。解释稳定性证书时使用保守上界或 fp32/fp64 检查。
