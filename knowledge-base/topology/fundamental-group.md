...
- 单连通要求路径连通且 $\pi_1(X,x_0)$ 平凡。
- 常见基本群：$\pi_1(S^1) = \mathbb{Z}$，$\pi_1(T^2) = \mathbb{Z}^2$，$\pi_1(S^n) = 0 \, (n \geq 2)$
- 覆盖分类：连通、局部路径连通、半局部单连通的空间，其带基点连通覆盖对应 $\pi_1$ 子群；无基点覆盖对应子群共轭类。
- Seifert–van Kampen 的 amalgamated-product 形式要求 $X=U\cup V$ 为开覆盖，$U,V,U\cap V$ 路径连通且基点在交中。
...
- **有限复形的 presentation**：用生成树外的边作生成元、二维面的边界作关系；Wirtinger 专指结图的常用表示。得到 presentation 不等于解决任意群的字问题。
- **环路检测**：图中的 cycle 可用遍历检测；带二维面的网格上 cycle 可能是可缩面边界，BFS/DFS 不能普遍判定不可缩性。
- **可微替代**：将"环路可缩性"转化为可微代理——如环路积分 $\oint \omega$（de Rham 上同调），闭形式积分非零即不可缩，这是可微的
- **覆盖空间的数值实现**：对于已知群结构（如 $\mathbb{Z}^n$），覆盖空间可以显式参数化为周期性拼接，可张量化
- 已知空间（圆、环面、有限图）的基本群可直接描述；一般有限 presentation 的字问题不可判定，不能由此声称所有基本群计算不可行。
