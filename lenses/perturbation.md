...
- 超出所证局部邻域的大扰动——需延拓、分段或全局界
- 需要大变形/全局结构分析——微扰只关心局部邻域
- 离散最优解可能跳变——可分析最优值或有间隙条件下的解稳定性
- 密码学不可区分/归约保证需配密码锚点；Lipschitz 与随机平滑鲁棒性证书本身仍是 AI 数学问题
...
3. **判断扰动类型**：正则扰动（通常可在所需区域作一致渐近展开，不要求级数收敛）vs 奇异扰动（主导项阶次改变）
