 ...
 **正则性与可辨识性**：期望负 Hessian 恒等式要求微分/积分可交换及适当支撑。冗余或不可辨识参数化的 Fisher 仅半正定；要成为 Riemannian 度量，须在建模方向正定。阻尼能形成可用预条件子，但会改变度量及其不变性。
...
## AI 设计翻译
- **Wasserstein 与 Fisher–Rao**：WGAN 类方法用 Wasserstein 目标比较分布，一般 GAN 未必如此。Fisher–Rao 为可辨识模型族赋予度量；比较两种几何需要共同的分布设定。
...
- **D2[~]**：无矩阵 Fisher 向量积可避免稠密存储；精确性取决于期望及 autodiff 实现。对角/Kronecker/低秩度量属于另外的近似。
- **D3[~]**：一些统计族有闭式测地线。一般高维 Fisher 测地线可能需要昂贵微分方程/边值求解；复杂度依模型而定，不是普遍不可计算。
- **D4[x]**：完整度量张量存储 $O(d^2)$，LLM 级完全不可能
- **D5[~]**：度量张量的条件数可能很大，低精度下不稳定
- **D6[~]**：样本、块及独立求解可并行；是否精确本身不决定并行性。迭代求解与坐标路径可能仍有依赖。
- **D7[~]**：Fisher 信息矩阵通常稠密；块对角近似（层间独立）引入结构化稀疏
- **D8[v]**：近似版本可融入优化器更新

**可行性判断**：LLM 规模避免稠密全参数度量，但应区分无矩阵乘积、结构化精确族及近似度量；按所需保证、可用算子和实测求解成本选择。

## 风险与失效条件
- **存储与访问**：稠密存储为 $O(d^2)$，但无矩阵访问无需保存所有元素。迭代次数、条件数及期望估计仍可能主导；避免存储不等于获得高效精确测地算法。
