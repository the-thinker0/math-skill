...
> Math Skill 是思考的 activator 和数学的锚点，不是知识百科。它的价值在于引导大模型激活正确的数学视角，而非穷举所有具体 AI 架构。

**三条原则**：

1. **回归数学本身**：knowledge-base/ 锚点应描述数学结构本身（流形、谱、熵、层上同调、李代数、伪随机函数族、格拉斯曼流形），而不是具体 AI 模块（diffusion、SSM、transformer 变体、具体 attention 变体）。具体 AI 架构会随研究演进快速过时，而数学结构是稳定的——以数学结构为锚点，skill 对任意未来的研究方向都保持引导能力。
2. **设计模式是翻译原型不是模板库**：design-patterns/ 提供的是"数学→AI 模块"的翻译范式示范，演示如何把数学结构变成 attention/loss/routing/representation/compression。使用者应把这些模式视为翻译方法论的范例，而不是可直接复制的模型仓库。遇到新问题应基于数学结构临时生成候选设计，而不是强行套用已有模式。
3. **引导思考而非替代思考**：skill 的产出应让使用者理解"为什么这个数学视角适合这个问题"，而不是给出一个现成答案。Knowledge Gap Protocol 是 skill 最区别于静态百科的核心能力——当现有锚点不覆盖时，生成临时知识卡继续引导，而不是停止或强行套用。这保证了 skill 对未预置具体架构的研究问题（如未来的新模型范式）依然有引导能力。
...
> 本 skill 不试图覆盖所有 AI 研究场景的具体架构，而是提供足够通用的数学激活视角，使其对扩散模型、状态空间模型、MoE、检索增强、对齐微调等任意研究方向都能起引导作用。当遇到 skill 未预置具体架构的研究问题时，应通过透镜路由 + 锚点激活 + 临时知识卡的流程完成引导，而非声明"不覆盖"。
