# 拓扑透镜

> 可以拉伸弯曲但不能撕裂——真正重要的性质在连续变形中不变

## 它是什么视角

拓扑是一种"忽略精确度量、只看定性结构"的眼光：当精确距离不重要时，拓扑捕获本质结构——连通性、洞的个数、维数。理想化的甜甜圈与带柄杯可以构造同胚，但“洞数相同”本身不是一般空间同胚的证明。核心洞察：形状比大小重要，同胚/同伦等价各自保留相应不变量；点云噪声鲁棒性另需滤流与稳定性界。上同调（特别是 Cech 上同调）还能检测"局部片段能否拼成全局相容对象"。

## 适合诊断什么问题

- 需要定性分类——不在乎"多大""多远"，只在乎"是否连通""有多少洞"
- 需要鲁棒性分析——先指定同胚/同伦等价或持续同调的稳定性假设；任意连续映射或数据扰动都可能改变拓扑
- 数据有标准统计无法捕获的形状——聚类、空洞、环状结构
- 检测全局一致性障碍——局部片段能否拼成全局相容对象

## 不适合什么问题

- 需要精确测量——拓扑只关心定性结构，不给距离/角度
- 度量性质是核心——拓扑等价空间可有完全不同的度量行为
- 需要精确数值决策——拓扑视角过粗，回答不了"多少"的问题

## 会路由到哪些知识域

- `topology/persistent-homology`：持续同调、滤流（filtration）、单纯复形、persistence barcode——从数据中提取尺度相关的拓扑特征（TDA 方法论）
- `topology/fundamental-group`：基本群、环路同伦类、覆盖空间——检测不可收缩环路与路径连通性结构

## 可能产生哪些 AI 设计

- **拓扑特征提取器**：从数据点云构建滤流，计算持续同调条形码
- **一致性检测器**：用 Cech 上同调 H^1 检测局部片段的全局相容性障碍
- **形状分类器**：利用不变量组合（chi, pi_1, beta_n）对空间做拓扑分类

## 推理协议

1. **明确等价标准**：确定同胚/同伦/微分同胚，区分什么可变（距离）什么不变（洞数）
2. **计算拓扑不变量**：欧拉示性数、连通分量、基本群、同调群、Betti 数
3. **用不变量分类**：χ 相同不单独决定曲面类型（还需连同可定向性）；π₁ 相同只是同伦等价的必要条件，远非充分（还需高阶同调/同伦群）
4. **构造拓扑模型**：点云建滤流/网络建图/系统建相空间
5. **验证等价性**：构造显式同胚或同伦等价（双向映射及复合同伦）；仅找到一个同伦或若干不变量相同不足以证明等价

## 验收标准

- 等价标准已明确（同胚/同伦/微分同胚）
- 不变量已计算：chi, pi_1, beta_n 至少给出可得的
- 分类结论已给出，标注了不变量是否完备
- 等价验证已完成（匹配/不匹配/不确定）
- 推理链条完整："因为[拓扑性质] → [结论]"
