...
name: axiomatization
description: |
  触发：当需要审查理论体系的假设合理性、构建新理论框架、发现理论内部矛盾，或分析某个观点的逻辑前提和隐含假设时调用。
  English: Trigger when examining the reasonableness of a theoretical framework's assumptions, building new theoretical frameworks, discovering internal contradictions in a theory, or analyzing the logical premises and hidden assumptions of an argument.
---

# 📐 公理化思想

> "从最少的基本假设出发，通过严格逻辑推理构建整个理论体系。"
> "Starting from the minimal set of basic assumptions, building an entire theoretical system through rigorous logical inference."
>
...
**任何理论体系都可以（且应该）被还原为一组基本公理，从这些公理出发，通过严格的逻辑推理，可以推导出该领域的所有定理。**

**Any theoretical system can (and should) be reduced to a set of fundamental axioms, from which all theorems in that domain can be derived through rigorous logical inference.**
...
- **用户已经接受某个理论框架，只需要应用**——公理已由用户选定
- **经验性问题**（需要实验数据而非逻辑推理）——公理化不能替代实证
- **构造性语境下使用经典推理工具**——若用户明确要求构造性证明，不应默认使用排中律或选择公理
...
公理体系的表达力取决于所选择的形式语言。必须在公理化之前明确语言框架：
...
检查公理之间是否互相矛盾。如果公理集合存在矛盾，则由爆炸原理可推导出任意命题，体系崩溃。
...
检查是否有某条公理可以由其他公理推导出来。如果存在，它就不是真正的公理，而是定理。
...
检查公理体系是否足以推导出该领域的所有重要结论。完备性的性质因逻辑阶数和理论内容而异：
...
|-------------|-------------------------------|---------------------------|
| 混淆公理和定理 | 把需要证明的命题当作不证自明的公理 | 明确区分：公理是起点，定理是终点 |
| 忽视隐式假设 | 论证中使用了未声明的假设，使推理不严密 | 审查每一步推理，暴露所有隐含假设 |
| 接受矛盾公理集 | 从矛盾的公理出发可以推出任意结论（爆炸原理）| 检验公理集合的相容性 |
| 把公理当真理 | 公理只是选择出发点的约定，不是绝对真理 | 公理的价值在于其推论的有用性 |
| 忽略公理变更的可能性 | 认为公理是唯一可能的选择 | 考虑改变公理会得到什么新理论 |
| 混淆一阶与二阶完备性 | 一阶逻辑有完备的证明系统（哥德尔完备性定理），但二阶逻辑没有完备的证明系统；二者"完备"含义不同 | 区分逻辑系统的完备性（proof-theoretic）与理论的完备性（model-theoretic），明确所用逻辑阶数 |
| 忽视构造性义务 | 在构造性语境中使用排中律或仅证明 ¬¬∃x.φ(x) 而不给出见证 | 构造性证明必须对 ∃x.φ(x) 给出显式见证 x₀；若语境为构造性，禁止默认使用排中律 |
| 假设所有不完备理论同样不完备 | 不同不完备理论的不可判定命题数量和性质差异巨大；有的只有少量独立命题（如 CH 对 ZFC），有的有大量 | 分析具体理论的不完备程度：哪些命题不可判定、不可判定命题的结构性质 |

## 操作规程 / Operating Procedure

当本 skill 被触发时，执行以下具体步骤并输出结构化分析：
