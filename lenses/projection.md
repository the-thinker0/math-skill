...
> 复杂整体可以分解为正交分量；正交不蕴含统计独立——冲突在子空间中暴露，信号与噪声在投影下分离。
...
- 子空间假设过强——数据不在低维子空间上
- 必须无损却打算丢弃非零分量的场景；保留两部分的正交分解本身是无损的
...
3. **构造投影算子**：$P = AA^H$（正交基）或 $P = A(A^HA)^{-1}A^H$（满列秩基；秩亏时用 Moore–Penrose 伪逆或正交基）
4. **执行分解**：$x = Px + (I-P)x$，分别评估两个分量的贡献
5. **验证正交性**：$P^2 = P$、$P = P^H$ 是否满足？基矩阵与求解过程的条件数是否可控（非满秩投影 P 自身是奇异的）？
