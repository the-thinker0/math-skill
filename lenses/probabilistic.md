...
- 要求在数据不足时作无假设的精确总体推断——没有统一的 n<5 禁用阈值；可给精确检验/贝叶斯分析，但需报告区间宽度与先验敏感性
- 数据完全缺失或严重损坏——无法做有意义的统计操作

## 会路由到哪些知识域

- `probability/entropy`：熵与互信息；集中与尾界见 `probability/concentration-inequality`，未覆盖的统计理论走缺口协议
- `probability/kl-divergence`：MLE、假设检验、贝叶斯推断、回归建模——从数据中提取规律
- 因果推断（暂无知识卡，按 Knowledge Gap Protocol 核验）：DAG、后门准则、do-演算——从关联到因果的推理框架
