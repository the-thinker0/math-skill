...
这是一种"测量者"的视角——为问题空间赋予度量张量（定义距离与角度），用曲率刻画空间的弯曲程度，用测地线研究局部长度驻定路径。核心信念：许多看似复杂的非线性问题，在正确的流形结构下会变得自然——度量、目标函数与算法共同影响优化；内在维数可指导压缩，但极限还依赖数据分布、失真度量与精度要求。
...
- 优化地形分析——区分损失 Hessian 与流形 Riemann 曲率；二次损失方向收敛因子取决于步长与 Hessian 特征值，不能只按曲率大小排序
- 表示学习中保持内蕴几何不变的重要性
...
- **differential-geometry/geodesic**：测地线方程、指数映射——流形上的局部长度驻定路径；全局最短性需额外条件
