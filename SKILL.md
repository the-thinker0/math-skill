...
本节同时优化两件事：**载入 token 更省**，**结论更有洞察**。

### 精益加载（省 token）
- **按节读取，不整文件读**：锚点先只读 `最小定义 + 核心公式 + 适用问题`；B/A 场景再按需补读 `AI 设计翻译 / 工程可行性 / 风险`；除非要引用出处或调试路由，**默认不读** `深入参考 / 路由扩展 / 可扩展方向`（这三段合计约占卡片尾部 ~15% 的 token）。
- **跳过重复数学段**：设计原型的 `数学思想来源 / 需要的数学知识` 常与已读锚点重叠（如 Eckart-Young、随机化 SVD）。若对应锚点已读，跳过这两段，直接读 `AI 模块形式 / GPU 可行性 / 风险`。
- **最小路径优先**：能读一张小卡就不读书稿或大文件（书稿约 4k token，仅深查时读）。
- **丢弃不改变结论的材料**：某透镜/锚点/原型若不影响最终结论，在回答里点名"已考虑但不影响结论"即可，不展开——既省输出 token 又证明已收敛。

### 结论契约（更有洞察）
结论性回答用下面 ≤5 项的紧凑契约，收敛到决策，不得只列视角或术语：
1. **一句结论**：成立吗 / 选什么 / 下一步。
2. **一个非显然洞察**：读者想不到、且**改变决策**的点（不是复述材料）。
3. **成立条件**：这句结论在哪个假设 / 边界内成立。
4. **一个反例或边界**：什么时候这套会失效。
5. **一个可证伪探针**：一个能推翻结论的具体实验 / 检查。

加载 ≥2 个透镜/锚点/原型时，额外强制一步：**指出它们的分歧点**（哪里相互矛盾、各自看不到什么），在此基础上丢弃不决定性的一方。
