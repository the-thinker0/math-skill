...
- Slater 充分条件：目标及不等式凸、等式仿射，且目标定义域相对内部存在满足等式并严格满足所有非线性不等式的点，在最优值有限等标准条件下有强对偶。
- 互补松弛：$\lambda_i^* g_i(x^*) = 0$（最优时，要么约束紧、要么乘子为零）
- 极小极大值：使用 $\inf_x\sup_{\lambda\ge0,\nu}L$ 与 $\sup_{\lambda\ge0,\nu}\inf_xL$。强对偶使两值相等；进一步写成 min/max 还要求最优值取到。
...
- **对偶间隙**：凸性加 Slater 是强对偶的充分条件，不是必要条件。增广 Lagrangian 算法需单独的收敛假设；非凸神经网络内层求解不会自动收敛到 KKT 点或消除全局对偶间隙。
- **对偶变量振荡**：$\lambda$ 的梯度上升步长不当会导致对偶变量振荡、原始不可行。解决：使用自适应步长（Adam 更新 $\lambda$）或增广 Lagrangian 的 $\rho$ 递增策略。
- **互补性诊断**：报告原始可行性、对偶可行性、驻点条件及 $\lambda_i g_i(x)$ 的尺度化残差。严格互补指活跃约束乘子 $\lambda_i>0$，是结构性质，不等同于浮点精确相等或固定 $10^{-6}$ 容差。
