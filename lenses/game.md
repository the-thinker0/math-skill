...
这是一种"对弈者"的视角——在多个决策者相互影响的环境中，每个参与者的最优行动取决于他人的行动，而他人的最优行动又反过来取决于你。它要求思维具有递归深度："我认为你认为我认为……"。核心不是优化自己的收益，而是找到没有单边有利偏离的策略组合——纳什均衡；它不自动保证学习动力学收敛或扰动稳定。
...
- 单一集中控制器可直接联合优化的任务；共同收益的多智能体仍可能有协调失败与信息约束
- 确定性问题，结果由自身行动唯一决定——不涉及他人反应
- 纯粹运气决定的结果，各方无策略选择
- 密码学的"攻击游戏"（attack game）——同名不同物：那是挑战者-敌手实验，用于形式化安全定义，不是博弈论；见 `../knowledge-base/cryptography/attack-game-framework.md`
...
4. **求解均衡**：占优策略→纯策略纳什→混合策略→子博弈完美，多重均衡检查帕累托支配关系（通常只是偏序）与选择假设
